...
function getLegendPtPerSong(rank) {
    ...
    const BASE_THRESHOLD = calculatePoints(99.5, '11.0'); // Changed: 99% -> 99.5%
    if (weight >= LEGEND_WEIGHT) return LEGEND_PT_CAP;
    // Power curve t^1.3 instead of linear
    const t = (weight - BASE_WEIGHT) / (LEGEND_WEIGHT - BASE_WEIGHT);
    return BASE_THRESHOLD + (LEGEND_PT_CAP - BASE_THRESHOLD) * Math.pow(t, 1.3);
}

const FOLDER_TIER_RATIOS = [
    { ratio: 1.0, name: 'Legend' },
...
function findScoreRate(rank, targetPt) {
    ...
    if (targetPt > maxPossible) return null;
    let lo = 66.666, hi = 100.0;
    for (let iter = 0; iter < 100; iter++) {
        const mid = (lo + hi) / 2;
        if (calculatePoints(mid, rank) < targetPt) lo = mid;
        else hi = mid;
    }
    return hi;
}
...
// Debug info
console.log("=== Updated: 99.5% base + t^1.3 power curve ===");
for (const f of folders) {
    const lpt = getLegendPtPerSong(f);
    const maxPt = calculatePoints(100, f);
    const legendRate = findScoreRate(f, lpt);
    console.log(`☆${f}: weight=${WEIGHTS[f]}, legendPtPerSong=${lpt.toFixed(2)}, maxPt=${maxPt.toFixed(2)}, Legend需要率=${legendRate ? legendRate.toFixed(2)+'%' : 'N/A'}`);
}
console.log("");

// Table - Legend only for each folder
