...
大ティア内のサブティア I..V は **等比分割**（公比 2^(1/5) ≒ 1.1487 = 約 +14.87%）。
等差で刻むと I→II に +20%、IV→V に +12.5% と 1 段上がるのに必要な伸び率がばらつくため、
対数グラフ上で等間隔になるよう `generateGeometricTieredRanks()` で刻んでいる。
閾値は 0.1 pt 単位に丸める。例（Master）: I=3200 / II=3675.8 / III=4222.4 / IV=4850.3 / V=5571.5

### 9.5 Folder Rank（フォルダ別ランク）

1 つの informal rank（= ☆11.0 ～ ☆13.1）に対してフォルダ単位・単曲単位のランクを算出する。
閾値は **☆ × ティア → 必要 score rate** の表で決まり、譜面固有の補正は入れない。

- `BLOCK_BOUND_RATES`: ☆（22 帯）× **ブロック境界 11 本**の必要 score rate。実測較正値
  （2026-09-22、Sparkle Shower 終了時点の `past_scores` 760,514 レコード + 歴代全国レコード）
  - 境界の並び: Legend / Mythic 1 / Ancient 1 / … / Intermediate 1 / Novice 1
  - Legend = その☆の譜面の歴代全国レコード rate の p10（外部基準）
  - Mythic 1 以下 = 譜面ごとの「上位 Q%」の rate の☆内中央値
    （Q = 1.2 / 3.8 / 8.0 / 13.5 / 20.5 / 28.5 / 37.5 / 47.5 / 60.0 / 80.0）
- `FOLDER_RANK_DEFS` は **51 ティア**（Legend + Mythic/Ancient/.../Novice のそれぞれ V..I）。
  名前・サブティア・色だけを持ち、**閾値は持たない**（添字が下の関数の index になる）
- `getFolderRankThresholdRateAt(index, informalRank)`: 添字 → 必要 score rate。
  ブロック境界（添字が 5 の倍数）は表の実測値そのもの、ブロック内の 5 サブティアは
  上下の境界をレート空間で 5 等分した位置
- `getFolderLegendRate(informalRank)`: 表の先頭列。☆11.0=99.93%, ☆12.2=99.25%, ☆13.1=94.95%
- `getFolderRankIndexByRate(rate, informalRank)`: 階段上の連続位置（0=Legend, 50=Novice 1）。
  ☆をまたいでも同じ尺度なので並び替えに使う

```
getFolderRankInfoByRate(rate, informalRank):        # 単曲ティア（レート判定）
  for i in 0..50:
    if rate >= getFolderRankThresholdRateAt(i, informalRank): return FOLDER_RANK_DEFS[i]
  return Beginner

getFolderRankInfo(totalPoints, informalRank, songCount):   # フォルダ合計 BEAT-PT 判定
  for i in 0..50:
    thresholdRate = getFolderRankThresholdRateAt(i, informalRank)
    if thresholdRate <= 66.666 break     # calculatePoints が 0 になり順序が壊れるため
    if totalPoints >= calculatePoints(thresholdRate, informalRank) * songCount:
      return FOLDER_RANK_DEFS[i]
  return Beginner
```

レート判定側は C 帯での打ち切りをしない（☆12.9 以上は Novice 1 の実測値が 66.666% を
下回るが、そこは実際に人が居る領域のため）。BEAT-PT 判定側だけ打ち切る。

較正のやり直し手順は `docs/単曲ティア較正.md` を参照。
...
saveHistoryLog(auth, SaveHistoryLogRequest):
  ...
  // 直前の upload がサーバー側で作った未確定ログ（tag=null / clientConfirmed=false）を
  // 10 分以内から 1 件引き当て、あればそれを仕上げる。無ければ従来どおり新規作成。
  // upload 側でも成長記録を作るようにしたため、ここで常に INSERT すると 1 回のアップロードで 2 行になる。
  log = scoreHistoryLogRepository
          .findFirstByUserAndTagIsNullAndClientConfirmedFalseAndUploadedAtGreaterThanEqualOrderByUploadedAtDesc(
              user, now - 10min)
        ?: new ScoreHistoryLog()
  log.clientConfirmed = true
...
- `WEIGHTS`, `RANKS`, `RATE_TIER_RANKS`, `FOLDER_RANK_DEFS`, `BLOCK_BOUND_RATES`, `SCORE_RATE_THRESHOLDS`
...
- `getFolderLegendRate / getFolderRankOffsetMax / getFolderRankThresholdRateAt / getFolderRankIndexByRate`
- `getLegendPtPerSong / getFolderRankInfo / getNextFolderRankInfo`
...
- **GET /api/analysis/fill-recommendation** — コスパ埋めレコメンド（auth）。`{top100Threshold, totalBeatPt, items:[{title, difficultyName, unplayed, breakEvenScore, achieveProbability, targetScore, targetGain, expectedGain, ...}]}` を `achieveProbability` 降順で返す（`expectedGain` = 達成率 × 達成時の増分）。式は [docs/コスパ埋めレコメンド.md](%E3%82%B3%E3%82%B9%E3%83%91%E5%9F%8B%E3%82%81%E3%83%AC%E3%82%B3%E3%83%A1%E3%83%B3%E3%83%89.md)
