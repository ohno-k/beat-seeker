...
| # | Method | Path | 認可 |
|---|--------|------|------|
| 7 | POST | `/api/admin/chart-tendencies/import` | Adm |
| 8 | POST | `/api/admin/chart-tendencies/import-json` | Adm |
| 9 | GET | `/api/admin/score-prediction` | Adm |
| 10 | GET | `/api/admin/similarity-debug` | Adm |
...
| MaintenanceView | メンテナンス中の全画面表示（`useMaintenance` が true のとき App の代わりに描画） |
...
### 15.17 useMaintenance.ts

```ts
MAINTENANCE_MODE = true   // マスタースイッチ。false でメンテナンス解除
checkBypass()             // ?maintenance=off / on（sessionStorage 'maintenanceBypass'）
isMaintenance: Ref<boolean>
```

- `main.ts` が `isMaintenance` を見て、`App.vue` の代わりに `views/MaintenanceView.vue` をマウントする。
  App をマウントしないため、メンテナンス中はスコア取得や認証チェックの API も一切飛ばない。
- 運営が動作確認したい場合は `?maintenance=off` を付けてアクセスする（そのタブの間だけ通常表示）。
  `?maintenance=on` で抜け道を解除できる。
