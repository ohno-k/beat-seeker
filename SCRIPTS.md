...
| [rebuild-draft-maxminus-zinrai.js](scripts/rebuild-draft-maxminus-zinrai.js) | ZINRAI 向け全曲再判定（**13.1 は `MANUAL_TOP` の手動指定枠**、残りは MAX-率順に active の帯別曲数で詰める。定員を消費するのは既に数値帯にいた曲だけで、Uncategorized からの新規参入曲は自分の順位が指す帯へ相乗りしその帯を 1 曲増やす。物量加点は 1800 以上で +0.1、以降 200 ごとに +0.1、**到達上限 12.8**・減点なし）。物量補正の効かせ方が異なる案A（上乗せ・帯サイズ変動）と案B（詰め直し・帯サイズ維持）を計算し、`--apply` で上書き前の draft を `profile:pre-zinrai-rebuild-YYYYMMDD`（`--backup=` で変更可）に保存 → 両案を `profile:ZINRAI-planA` / `profile:ZINRAI-planB` に保存（既存は無条件上書き）→ `--plan=A|B`（既定 B）を draft に書き込む。`--quota=13.0:10,12.9:20` で帯別定員を上書きでき（帯:曲数、指定しない帯は active の曲数）、増減分は指定した最下帯のすぐ下の帯が吸収する（例では 12.8 が 37 → 25）。吸収を複数帯に分散させたいときは吸収先も明示する（例: `--quota=13.0:10,12.9:20,12.8:33,12.7:44,12.6:64,12.5:62,12.4:75`、合計が 0 なら自動吸収なし）。`--no-notes-bonus` で物量加点を無効化（帯は MAX-率順と定員だけで決まり、案A = 案B = ベース配置。2026-09-11 の現 draft はこのモード）。active には触れない。2026-08-19 初回、2026-09-11 に最新データで再実行（draft = 案B）、同日 13.0 = 10 / 12.9 = 20 に拡張 → 全帯を山なりの定員に変更し、上下の移動が釣り合うよう裾を広げた（中心 11.8・上側 σ 8・下側 σ 6.5、現 draft。docs 次回作ZINRAI移行計画 §8.1 に定員の実数）。 | 出力: `data/zinrai_rebuild_changes.json`, `data/zinrai_rebuild_report.md` |
| [place-uncategorized-draft.js](scripts/place-uncategorized-draft.js) | active の Uncategorized に居る曲を draft の数値帯へ入れ直す（draft で既にどこかに置かれていても外して置き直す）。配置条件はプレイ人数 `n > --min-plays`（既定 200、n は MAX-率の分母）。ベース帯 = draft 既存曲との MAX-率順位（昇順→平均スコアレート昇順）が指す帯へ相乗り（既存曲は動かさない）、物量加点は ZINRAI 移行計画と同じ（1800 以上 +0.1、以降 200 ごと +0.1、到達上限 12.8、減点なし）、13.1 は手動枠なので上限 13.0。データ無し・n 閾値以下は Uncategorized に残置。デフォルト dry-run、`--apply` で現 draft を `profile:pre-uncat-place-YYYYMMDD`（`--backup=` で変更可）に保存してから draft を書き換え。active には触れない。2026-09-09 に 21 曲を配置済み。 | 出力: `data/uncat_place_report.md` |
| [backfill-difficulty-revisions.js](scripts/backfill-difficulty-revisions.js) | 更新履歴「難易度改訂」の第1〜5版を `difficulty_revisions` へ補填（2026-09-09 実施済み・一回限り）。自動記録機能のデプロイ前に旧コードで適用された第5版（Uncategorized 21 曲の配置）を、`profile:pre-uncat-place-20260909` から復元した適用前 active と現 active の差分として再現。`ON CONFLICT DO NOTHING` なので既存版はスキップ。デフォルト dry-run、`--apply` で INSERT。 | 出力: コンソールのみ |
