...
public class ScoreController {
    ...
    private static final double[][] SCORE_RATE_THRESHOLDS = {
        {77.77, 1.0}, {88.89, 2.0}, {94.44, 4.0}, {97.22, 8.0},
        {98.61, 16.0}, {99.31, 32.0}, {99.65, 64.0}, {99.83, 128.0},
        {99.91, 256.0}, {100.0, 512.0}
    };

    private double calcRatePt(double scoreRate) {
        if (scoreRate < SCORE_RATE_THRESHOLDS[0][0]) return 0.0;
        if (scoreRate >= 100.0) return 512.0;
        for (int i = 0; i < SCORE_RATE_THRESHOLDS.length - 1; i++) {
            if (scoreRate < SCORE_RATE_THRESHOLDS[i + 1][0]) {
                double t = (scoreRate - SCORE_RATE_THRESHOLDS[i][0])
                         / (SCORE_RATE_THRESHOLDS[i + 1][0] - SCORE_RATE_THRESHOLDS[i][0]);
                return SCORE_RATE_THRESHOLDS[i][1] + t * (SCORE_RATE_THRESHOLDS[i + 1][1] - SCORE_RATE_THRESHOLDS[i][1]);
            }
        }
        return 0.0;
    }
    ...
    /**
     * Aggregates which songs appear in users' RATE-PT top-100.
     * Accepts song max-scores map from frontend to avoid sending all raw scores.
     */
    @PostMapping("/rate-song-ranking")
    public ResponseEntity<List<Map<String, Object>>> getRateSongRanking(
            @RequestBody Map<String, Integer> songMaxScores) {

        List<Map<String, Object>> rawScores = scoreRepository.findAllUserAnotherAndLeggendariaScores();

        // userId -> list of [title, diffName, ratePt]
        Map<Object, List<Object[]>> userScores = new HashMap<>();
        for (Map<String, Object> score : rawScores) {
            Object userId = score.get("userId");
            String title = (String) score.get("title");
            String diffName = (String) score.get("difficultyName");
            int diffCode = "ANOTHER".equals(diffName) ? 4 : 10;
            Integer maxScore = songMaxScores.get(title + "_" + diffCode);
            if (maxScore == null || maxScore == 0) continue;
            Object scoreVal = score.get("score");
            if (scoreVal == null) continue;
            double scoreRate = ((Number) scoreVal).intValue() * 100.0 / maxScore;
            double ratePt = calcRatePt(scoreRate);
            if (ratePt <= 0) continue;
            userScores.computeIfAbsent(userId, k -> new java.util.ArrayList<>())
                      .add(new Object[]{title, diffName, ratePt});
        }

        // For each user take top-100, aggregate per song
        Map<String, long[]> songStats = new HashMap<>();   // key -> [count, totalRatePt*1000]
        Map<String, Object[]> songInfo = new HashMap<>();  // key -> [title, diffName]
        for (List<Object[]> scores : userScores.values()) {
            scores.sort((a, b) -> Double.compare((double) b[2], (double) a[2]));
            for (int i = 0; i < Math.min(100, scores.size()); i++) {
                Object[] s = scores.get(i);
                String key = s[0] + "_" + s[1];
                songStats.computeIfAbsent(key, k -> new long[]{0, 0});
                songStats.get(key)[0]++;
                songStats.get(key)[1] += (long) ((double) s[2] * 1000);
                songInfo.put(key, new Object[]{s[0], s[1]});
            }
        }

        List<Map<String, Object>> result = new java.util.ArrayList<>();
        for (Map.Entry<String, long[]> e : songStats.entrySet()) {
            long[] st = e.getValue();
            Object[] info = songInfo.get(e.getKey());
            Map<String, Object> row = new HashMap<>();
            row.put("title", info[0]);
            row.put("difficultyName", info[1]);
            row.put("userCount", (int) st[0]);
            row.put("avgRatePt", st[0] > 0 ? (st[1] / 1000.0) / st[0] : 0.0);
            result.add(row);
        }
        return ResponseEntity.ok(result);
    }
