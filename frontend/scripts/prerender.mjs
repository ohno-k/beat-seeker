...
<article style="${articleStyle}">
  ...
  <ul style="padding-left:24px;line-height:2;margin-top:8px;">
    <li><strong>取得する情報:</strong> (1) ユーザーが登録した IIDX ID・パスワード（ハッシュ化して保存）・ユーザー名・段位・アリーナランク等のプロフィール情報、(2) パスワード再設定用に任意で登録するメールアドレス、(3) ユーザーが登録したスコアデータ（CSV・ブックマークレット・INFINITAS 画面取り込み）およびリザルト画像、(4) お問い合わせチャット等でユーザーが送信した内容、(5) サーバー運用・不正防止のために自動的に記録されるアクセスログ（IP アドレス・ブラウザ情報・アクセス日時等）。</li>
    ...
    <li><strong>INFINITAS 画面取り込み:</strong> 画面共有の映像はお使いのブラウザ内で解析され、読み取り結果（スコア等）のみがサーバーに送信されます。映像や画像そのものが送信・保存されることはありません。</li>
