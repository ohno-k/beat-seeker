  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <div class="relative bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
          <!-- ヘッダ: タイトル + Lv/XP + 閉じる -->
          <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-3">
            <div class="w-9 h-9 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0">
            ...
            <div class="flex-1 min-w-0">
              <h3 id="rank-quiz-modal-title" class="text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-widest">非公式難易度クイズ</h3>
          ...
          <div class="p-5">
            ...
            <template v-else-if="sessionFinished">
              <div class="text-center space-y-3">
                ...
                <h4 class="text-lg font-black text-slate-800 dark:text-slate-100">セッション完了！</h4>
                <p class="text-sm text-slate-500 dark:text-slate-400">
                  正答 <span class="font-black text-slate-800 dark:text-slate-100">{{ correctCount }}/{{ questions.length }}</span>
                  ・ 獲得 <span class="font-black text-amber-500">+{{ xpGainedTotal }} XP</span>
                </p>
                <div v-if="sessionLeveledUp" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-400 to-yellow-400 text-white font-black text-xs shadow-sm">
              ...
              <ul class="mt-5 space-y-1.5 text-xs">
                ...
                >
                  <span :class="a.correct ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'" class="font-black w-4 text-center">
                    {{ a.correct ? '○' : '×' }}
                  </span>
                  ...
                  <span class="font-black text-slate-700 dark:text-slate-200 tabular-nums">★{{ a.correctRank }}</span>
                  <span v-if="!a.correct" class="text-[10px] text-rose-500 tabular-nums">→ ★{{ a.chosenRank }}</span>
                  <span v-if="a.isReview" class="text-[8px] font-black px-1 py-px rounded bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">復習</span>
                </li>
              </ul>
              <div class="flex gap-2 mt-5">
                <button
                  @click="$emit('close')"
                  class="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
                >閉じる</button>
                <button
                  @click="restart"
                  class="flex-1 px-4 py-2.5 rounded-xl text-sm font-black text-white bg-gradient-to-r from-indigo-500 to-purple-500 hover:shadow-lg hover:shadow-indigo-500/20 transition-all"
                >もう1セット</button>
              </div>
            </template>
            ...
            <template v-else-if="currentQuestion">
              ...
              <div class="flex items-center justify-between text-[10px] font-bold text-slate-500 dark:text-slate-400 mb-3">
                ...
                <span v-if="currentQuestion.isReview" class="px-1.5 py-0.5 rounded bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 text-[9px] font-black">復習問題 +{{ 20 }}XP</span>
                <span v-else class="text-slate-400">新規問題 +{{ 10 }}XP</span>
              </div>

              <!-- 設問 -->
              <div class="text-center space-y-2 mb-4">
                <p class="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">この譜面は ★いくつ？</p>
                <p class="text-xl font-black text-slate-900 dark:text-white break-words leading-tight">{{ currentQuestion.title }}</p>
                <span
                  class="inline-block px-2.5 py-0.5 rounded-md text-[10px] font-black tracking-widest"
              ...
              <div class="grid grid-cols-2 gap-2">
                <button
                  ...
                  class="px-4 py-3 rounded-xl text-base font-black tabular-nums border-2 transition-all"
                  :class="[
                    !currentAnswered
              ...
              <!-- 結果フィードバック -->
              <div v-if="currentAnswered" class="mt-4 px-3 py-3 rounded-xl text-sm font-bold"
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  ...
                  <span v-if="lastResult?.leveledUp" class="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-400 text-white">
                    🎉 Lv UP! → {{ lastResult.newLevel }}
                  </span>
                </div>
                <p class="text-[11px] font-medium mt-1 text-slate-600 dark:text-slate-300">
                  覚え方: <span class="font-black text-slate-800 dark:text-slate-100">{{ currentAnswered.title }}{{ currentAnswered.difficultyName === 'LEGGENDARIA' ? '[L]' : '' }} = ★{{ currentAnswered.correctRank }}</span>
                </p>
                <button
                  type="button"
                  @click="next"
                  class="mt-3 w-full px-4 py-2 rounded-lg text-sm font-black text-white bg-gradient-to-r from-indigo-500 to-purple-500 hover:shadow-md transition-all"
