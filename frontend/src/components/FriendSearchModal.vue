...
const { searchUsers, sendFriendRequest, isLoading, error } = useFriends();
...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200" @click.self="emit('close')">
        ...
        <form @submit.prevent="handleSearch" class="flex gap-2">
          <div class="relative flex-1">
            ...
            <input 
              type="text" 
              v-model="searchQuery"
              placeholder="表示名 または IIDX ID (XXXX-XXXX)"
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                <span class="font-bold text-slate-900 dark:text-white truncate">{{ result.displayName }}</span>
                <span class="text-[10px] bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded text-slate-500 dark:text-slate-400 font-mono">{{ result.iidxId }}</span>
              </div>
              <div class="flex items-center gap-3 mt-1">
                ...
                <span class="text-[10px] text-slate-400 dark:text-slate-500">最終更新: {{ formatDate(result.lastUploadedAt) }}</span>
              </div>
            </div>
