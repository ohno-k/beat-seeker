...
<template>
  <div class="w-full max-w-2xl mx-auto flex flex-col items-center">
    ...
    <!-- Tab Navigation -->
    <div class="flex w-full mb-4 bg-slate-100 p-1 rounded-xl">
      <button 
        class="flex-1 py-2 text-sm font-medium rounded-lg transition-all"
        :class="activeTab === 'file' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
      ...
      <button 
        class="flex-1 py-2 text-sm font-medium rounded-lg transition-all"
        :class="activeTab === 'text' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'"
    ...
    <div class="w-full relative min-h-[300px]">
        ...
        <transition name="fade">
            <div 
            v-if="activeTab === 'file'"
            class="absolute inset-0 w-full h-full p-12 border-2 border-dashed rounded-2xl transition-all duration-200 flex flex-col items-center justify-center cursor-pointer bg-white shadow-sm"
            :class="{
        ...
        <transition name="fade">
            <div 
            v-if="activeTab === 'text'"
            class="absolute inset-0 w-full h-full p-6 border border-slate-200 rounded-2xl bg-white shadow-sm flex flex-col"
            >
            ...
            <textarea
                ...
                class="flex-1 w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none font-mono text-sm mb-4"
                placeholder="バージョン,タイトル,ジャンル,アーティスト..."
            ></textarea>
            
            <button 
                class="w-full py-3 bg-slate-800 hover:bg-slate-900 text-white font-medium rounded-lg transition-all shadow-md flex items-center justify-center gap-2"
            ...
            <p v-if="!pastedCsvText.trim()" class="text-xs text-slate-400 text-center mt-3">
